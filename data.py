...
def get_image_mask_pairs(base_folder):
    ...
    if not os.path.exists(images_folder):
        print(
            f"Note: No images folder found in {base_folder} we use only mask images if it works"
        )
...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Handle different TIFF formats (grayscale vs RGB)
            if len(mask_img.shape) > 2:
                mask_img = mask_img[:, :, 0]  # Take first channel if multi-channel
        except Exception as e:
            raise FileNotFoundError(
                f"Mask file not found or cannot be read: {mask_file}. Error: {str(e)}"
            )

        mask = (mask_img > 0).astype("float32")
        ...
        try:
            if self.gen_nc == 1:  # 2D
                ...
                img = io.imread(img_file)
                # Ensure it's RGB for 3D case
                if len(img.shape) == 2:
                    img = np.stack([img, img, img], axis=-1)
                img = img.astype("float32")
        except Exception as e:
            raise FileNotFoundError(
                f"Image file not found or cannot be read: {img_file}. Error: {str(e)}"
            )
